...
def get_spikes(rat_name, mod_name, day_name = '', sess_name = '', bType = None, bSmooth = True, 
    ...
    if bType:    
        ...
        if len(day_name)>0:
            is_conj = f['is_conj_' + string_func(rat_name) + mod_name + '_' + day_name]
        else:
            is_conj = f['is_conj_' + string_func(rat_name) + mod_name + day_name]
        f.close()
        if bType == 'conj':
    ...
    if sess_name.find('_')>-1: 
        ...
        ind = f[string_func(rat_name) + mod_name + '_ind']
        f.close()
    ...
    if string_func(sess_name[:3]) in ('SWS', 'REM'):
        times = times_all['rat_' + string_func_2(rat_name) + '_sleep' + day_name]
    elif string_func(sess_name[:3]) == 'WW2':
        times = times_all['rat_' + string_func_2(rat_name) + '_' + sess_name + day_name]        
    else:
        times = times_all['rat_' + string_func_2(rat_name) + '_' + sess_name[:2] + day_name]        
    ...
    if not bBinned:
        ...
        if (rat_name =='S') & (sess_name == 'WW'):
            times2 = times_all['rat_' + string_func_2(rat_name) + '_' + sess_name + '2' + day_name]        
        ...
        if sess_name[:2] in ('OF', 'WW'):
            ...
            start = np.array(times_all['rat_' + string_func_2(rat_name) + '_' + sess_name[:3] + day_name])[:,0]
            start = start[start<max_time0]

            end = np.array(times_all['rat_' + string_func_2(rat_name) + '_' + sess_name[:3] + day_name])[:,1]
        ...
        if bSmooth:
            sigma = 5000
            if string_func(sess_name[:3]) == 'SWS':
                sigma = 2500
        ...
        if sess_name[:2] in ('OF', 'WW'):
            ...
            start = np.array(times_all['rat_' + string_func_2(rat_name) + '_' + sess_name[:3] + day_name])[:,0]*res
            start = start[start<max_time]

            end = np.array(times_all['rat_' + string_func_2(rat_name) + '_' + sess_name[:3] + day_name])[:,1]*res
...
def load_pos(rat_name, sess_name, day_name = '', bSpeed = False, folder = ''):    
    ...
    if string_func(sess_name) in ('SWS', 'REM'):
        times = times_all['rat_' + string_func_2(rat_name) + '_sleep' + day_name]
    else:
        times = times_all['rat_' + string_func_2(rat_name) + '_' + sess_name + day_name]        
    ...
    if (string_func(rat_name) == 'R') & (day_name == 'day1'):
        if sess_name =='OF':
            valid_times = np.concatenate((np.arange(0, (14778-7457)*res/dt),
                                          np.arange((14890-7457)*res/dt, (16045-7457)*res/dt))).astype(int)
        elif sess_name == 'WW':
    ...
    if bSpeed:
        ...
        speed = np.concatenate(([speed[0]],speed))
        if (string_func(rat_name) =='S') & (sess_name == 'WW'):
